...
2. Open Apps Script and replace the editor contents with `google-sheets.gs` from this folder. The script writes to the spreadsheet identified by `SHEET_ID`, so it does not depend on whichever spreadsheet tab happens to be active.
3. In **Project Settings → Script properties**, add:
   - `SHEET_ID`: the ID between `/d/` and `/edit` in the destination Sheet URL.
